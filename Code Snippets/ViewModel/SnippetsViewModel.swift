...
class SnippetsViewModel: ObservableObject {
    ...
    func addSnippet(name: String, category: String, code: String) {
        let stringID = UUID().uuidString
        guard let userID = auth.currentUser?.uid else {return }
        let firebaseSnippet = Snippets(id: stringID, name: name, category: category, code: code)
        
        do {
            try FirebaseManager.shared.database.collection("users").document(userID).collection("categories").document(stringID).collection("snippets").document(stringID).setData(from: firebaseSnippet)
            fetchSnippets(categoryID: stringID)
        } catch {
